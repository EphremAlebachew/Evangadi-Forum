...
import AnswerQuestion from "../../Componets/AnswerQuestion/AnswerForm";
  ...
  const questionByPostId = async () => {
    try {
      const question = await axios.get(
        `http://localhost:4004/api/questions/${params.id}`
      );
  ...
  const answersByQuestionId = async () => {
    try {
      const answersRes = await axios.get(
        `http://localhost:4004/api/answer/${question?.question_id}`
      );
